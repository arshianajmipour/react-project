import React from "react";
import "./Home.css";
import * as bs from 'bootstrap/dist/css/bootstrap.css';
import Card from 'react-bootstrap/Card';
import Button from "react-bootstrap/Button";
function Home() {
    return(
<div className="home">
    <Card style={{ width: '18rem' }} className="home-card">
        <Card.Body>
            <Card.Img variant="top" height="255"
                      src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRKja0lwgwMMD7S9fEjyfgffmqbkkc8A5md7h4MKxrAIVxnyK7jWgbVE-PR7PvWtzemWLg&usqp=CAU"  />
            <Card.Title>React Course</Card.Title>
            <Card.Text>
                price:10$<br/>
                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ad numquam quisquam repellendus?
            </Card.Text>
            <Button variant="primary">buy this course</Button>
        </Card.Body>
    </Card>
    <Card style={{ width: '18rem' }} className="home-card">
        <Card.Body>
            <Card.Img variant="top" src="data:image/png;base64," />
            <Card.Title>javascript Course</Card.Title>
            <Card.Text>
                price:10$<br/>
                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eius, porro!
            </Card.Text>
            <Button variant="primary">buy this course</Button>
        </Card.Body>
    </Card>
    <Card style={{ width: '18rem' }} className="home-card">
        <Card.Body>
            <Card.Img variant="top" src="https://lh4.googleusercontent.com/XHxewbpGnE_Z-15z217NbvazviJIexZw-LUCk5qiw8zS-ZwK4QR2L5LXWd5yeOEs6334vq4I78aqp8IrWf7w-NE6RC1sdWfxW-6kAX59St-TS8-wDcj9NqIVw4AyOGy3z2dNo5cf" />
            <Card.Title>bootstrap-react Course</Card.Title>
            <Card.Text>
                price:5$<br/>
                Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </Card.Text>
            <Button variant="primary">buy this course</Button>
        </Card.Body>
    </Card>
</div>
    )
}
export default Home;